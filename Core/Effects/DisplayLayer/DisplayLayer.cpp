...
DisplayLayer::DisplayLayer(std::string name):
	EffectGL(name,"DisplayLayer")
{
	...
	layer_bound = fp->uniforms()->getGPUint("layer_bound");
}
...
void DisplayLayer::display(const glm::vec4 & box, int layer)
{
		layer_bound->Set(layer);
