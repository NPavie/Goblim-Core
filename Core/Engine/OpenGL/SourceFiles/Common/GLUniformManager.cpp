...
std::vector<GPUBuffer*> GLUniformManager::sharedBufferList = std::vector<GPUBuffer*>();
...
void GLUniformManager::parseUniforms()
{
	...
	for(GLuint i=0; i< (GLuint) activeBlockUniforms; i++)
	{
		...
		bool shared = false;
		if (std::string("LightingBuffer").compare(name) == 0) shared = true;
		if (std::string("matricesBuffer").compare(name) == 0) shared = true;
		...
		if (std::string("LightingBuffer").compare(name) == 0)
			B->m_BindingPoint = LIGHTING_UNIFORM_BINDING;

		if (std::string("matricesBuffer").compare(name) == 0)
			B->m_BindingPoint = MATRICES_UNIFORM_BINDING;
...
{
    // when accessing for the first time to the lighting buffer, we attach each global buffer to a block
    if(firstAccess){
        for(unsigned int i = 0;i < listBlocks.size();i++){
            for(unsigned int j = 0; j < sharedBufferList.size(); ++j){
                if (listBlocks[i]->getName().compare(sharedBufferList[j]->name()) == 0)
                {
                    listBlocks[i]->attachBuffer(sharedBufferList[j]);
                }
            }
        }
        firstAccess = false;
...
void GLUniformManager::storeInSharedBuffersList(GPUBuffer* ubo){
    GLUniformManager::sharedBufferList.push_back(ubo);
}
