	...
	Camera(name)
{
	...
	m_ProjBuffer->create(sizeof(ProjectionMatrices) , GL_UNIFORM_BUFFER  ,GL_DYNAMIC_COPY);
	m_ProjBuffer->setBindingPoint(MATRICES_UNIFORM_BINDING);
...
void CameraGL::updateBuffer()
{
	...
	glBindBuffer(GL_UNIFORM_BUFFER, 0); // A mon avis, si le buffer n'est pas release, le map n'est pas valider par l'implémentation AMD
