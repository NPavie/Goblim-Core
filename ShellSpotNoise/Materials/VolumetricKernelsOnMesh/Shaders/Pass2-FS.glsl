...
void main()
{
	...
	vec3 pointInCamera = (surfaceKernel_toViewSpace * vec4(positionFinale,1.0)).xyz;
	float oitWeight = weightFunction(abs(-pointInCamera.z));// * shellWeightFunction(fragmentImpulse.heightFromGround / gridHeight);
	float newAlpha = accumulateur * oitWeight * colorInProfile.a;

	Color = vec4(colorAccumulation.xyz * newAlpha,newAlpha);
...
kernelProfile currentProfile = Kernels[surfaceKernel.profileID];
	...
	if(colorAccumulation.a < 1.0) discard;
	if(meanDepth > 1.0) meanDepth /= colorAccumulation.w;
	gl_FragDepth = meanDepth;

	if(colorAccumulation.a < 0.01) discard;

	if(colorAccumulation.a > 1.0) colorAccumulation.rgb /= colorAccumulation.a;
	if(colorAccumulation.a > 1.0) colorAccumulation.a = 1.0;
