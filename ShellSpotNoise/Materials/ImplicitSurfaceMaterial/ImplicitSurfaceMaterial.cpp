...
ImplicitSurfaceMaterial::ImplicitSurfaceMaterial(std::string name, KernelList* kernelsToRender, GPUFBO* accumulatorFBO, GPUTexture* depthTex)
	: MaterialGL(name, "ImplicitSurfaceMaterial")
{
...
ImplicitSurfaceMaterial::~ImplicitSurfaceMaterial()
{

}

void ImplicitSurfaceMaterial::render(Node *o)
{
	if (m_ProgramPipeline)
	{
		...
		if (useBestFragment)
		{
			...
			if (depthTexture != NULL) depthTexture->bind(fp_smp_depth->getValue());
...
void ImplicitSurfaceMaterial::update(Node *o, const int elapsedTime)
{
