...
// Rendu normale pour fragment contributif le plus proche
void main()
{
	...
	// Discard si pointInCamera.z <= minDepth ou pointInCamera.z > maxDepth
	float halfRange = (maxDepth - minDepth) / 2.0;
	if(pointInCamera.z <= (minDepth-halfRange) || pointInCamera.z >= (maxDepth+halfRange) ) discard;
	...
	//Color = vec4(kernelContrib.rgb * kernelContrib.a,kernelContrib.a);
	Color = kernelContrib;
}
